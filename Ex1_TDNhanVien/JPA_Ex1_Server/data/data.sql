...
select ca.candidate_id, count(e.company_name) from candidates ca join experiences e on ca.candidate_id = e.candidate_id
group by ca.candidate_id
 
SELECT e.candidate_id, e.position_id, 
...
HAVING DATEDIFF(year, MIN(e.from_date), MAX(e.to_date)) = (
    SELECT MAX(tgLamViec) AS max_tgLamViec
    FROM (
        SELECT e.candidate_id, e.position_id, 
               DATEDIFF(year, MIN(e.from_date), MAX(e.to_date)) AS tgLamViec
        FROM experiences e
...
SELECT e.candidate_id, e.position_id, 
       DATEDIFF(year,e.from_date, e.to_date) AS tgLamViec
FROM experiences e
GROUP BY e.candidate_id, e.position_id,e.from_date, e.to_date

--Tìm danh sách các ứng viên đã làm việc trên một vị trí công việc nào đó có thời gian làm lâu nhất. 
SELECT e.candidate_id, e.position_id, ca.full_name, p.name,
       DATEDIFF(year, MIN(e.from_date), MAX(e.to_date)) AS tgLamViec
FROM experiences e
JOIN positions p ON e.position_id = p.position_id
Join candidates ca on e.candidate_id = ca.candidate_id
GROUP BY e.candidate_id, e.position_id,ca.full_name, p.name
HAVING DATEDIFF(year, MIN(e.from_date), MAX(e.to_date)) >= (
...
select ce.* from  certificates ce join candidates ca on ce.candidate_id = ca.candidate_id
group by ce.candidate_id
having ca.candidate_id = (select candidate_id from candidates)


SELECT ca.candidate_id, ca.full_name,
    STRING_AGG(ce.certificate_id, ', ') WITHIN GROUP (ORDER BY ce.certificate_id) AS degrees_list
FROM candidates ca
LEFT JOIN certificates ce ON ca.candidate_id = ce.candidate_id
GROUP BY ca.candidate_id, ca.full_name;
